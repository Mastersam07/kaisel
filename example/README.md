...
| --- | --- |
| `lib/main.dart` | The main example: bottom-nav `GateBranchedShell` with per-tab typed routes, modal flows, modules, and URL deep-linking via codec |
...
The main example. Bottom-nav `GateBranchedShell` with three tabs
(`Home`, `Discover`, `Profile`), each with its own typed sealed
route hierarchy and back stack. Includes a `ConfirmAddToCart` modal
flow (`router.run<bool>(...)`), a `GateModuleMount` for the
Checkout module, and URL deep-linking via `GateConfigCodec` →
...
- **Films** tab uses `GateBranch.adaptive` with a master-detail
  builder. Resizing flips this tab's layout between stacked and
  side-by-side.
- **Settings** tab uses a regular `GateBranch`. Same layout at all
  widths.
