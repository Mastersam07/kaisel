// Adaptive layout demo for the gate router.
//
...
sealed class BookRoute extends GateRoute {
  const BookRoute();
}
...
class _AdaptiveBookAppState extends State<AdaptiveBookApp> {
  late final GateRouter<BookRoute> _router;
  late final GateRouterDelegate<BookRoute> _delegate;

  @override
  void initState() {
    super.initState();
    _router = GateRouter<BookRoute>(initial: const BookList());
    _delegate = GateRouterDelegate<BookRoute>.adaptive(
      router: _router,
  ...
  GatePageResult _buildPage(
    BookRoute route,
    GateStackContext<BookRoute> stack, {
    required bool wide,
  }) {
    return switch ((route, stack.previous, wide)) {
      ...
      (BookList(), _, true) => const GateStandalonePage(
      ...
      (BookDetail(:final id), _, _) => GateStandalonePage(
        BookDetailScreen(id: id),
      ),

      // Narrow + List: plain list.
      (BookList(), _, _) => const GateStandalonePage(BookListScreen()),
      ...
      (BookReviewsRoute(:final bookId), _, _) => GateStandalonePage(
...
class _NoopParser extends RouteInformationParser<GateConfig<BookRoute>> {
  _NoopParser(this.router);

  final GateRouter<BookRoute> router;

  @override
  Future<GateConfig<BookRoute>> parseRouteInformation(
    RouteInformation routeInformation,
  ) async => GateConfig<BookRoute>(mainStack: router.stack);
}
