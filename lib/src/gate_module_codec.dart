...
abstract class UntypedModuleStackCodec {
  ...
  List<String> encodeAny(List<GateRoute> stack);
  ...
  /// [GateRoute] for the composer.
  List<GateRoute>? decodeAny(List<String> segments);
}
...
abstract class ModuleStackCodec<R extends GateRoute>
  ...
  @override
  List<String> encodeAny(List<GateRoute> stack) => encode(stack.cast<R>());

  @override
  List<GateRoute>? decodeAny(List<String> segments) {
    final result = decode(segments);
    return result?.cast<GateRoute>();
  }
}
...
@immutable
class ModuleMount<HostR extends GateRoute> {
  ...
  /// is a [GateModuleConfig] holding the module's decoded stack.
...
/// A [GateConfigCodec] that composes a base codec with one or more
/// module mounts.
///
...
class ConfigCodecWithModules<R extends GateRoute>
  ...
  final GateConfigCodec<R> baseCodec;
      ...
      if (_segmentsStartWith(segments, prefix)) {
        ...
        if (moduleStack != null && moduleStack.isNotEmpty) {
          return GateConfig<R>(
            ...
            nestedState: GateModuleConfig(stack: moduleStack),
          );
        }
