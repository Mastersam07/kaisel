// Route-pair transitions demo for the gate router.
//
...
// receives a `GatePageWrapperContext<R>` and pattern-matches on the
...
class _TransitionsAppState extends State<TransitionsApp> {
  ...
  void initState() {
    super.initState();
    _router = GateRouter<AppRoute>(initial: const Home());
  ...
  Page<Object?> _pageWrapper(GatePageWrapperContext<AppRoute> ctx) {
...
class _NoopParser extends RouteInformationParser<GateConfig<AppRoute>> {
  ...
  ) async => GateConfig<AppRoute>(mainStack: _router.stack);

  GateRouter<AppRoute> get _router => router;
}
