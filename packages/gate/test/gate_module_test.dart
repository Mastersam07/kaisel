...
sealed class _OtherRoute extends GateRoute {
  const _OtherRoute();
}
...
void main() {
  group('GateModuleConfig', () {
    ...
    test('differs when stack differs', () {
      final a = GateModuleConfig(stack: const [_Cart()]);
      final b = GateModuleConfig(stack: const [_Cart(), _Shipping()]);
      expect(a, isNot(equals(b)));
    });

    test('stack is unmodifiable', () {
      final c = GateModuleConfig(stack: [const _Cart()]);
      expect(() => c.stack.add(const _Shipping()), throwsUnsupportedError);
    });

    test('rejects an empty stack', () {
      expect(
        () => GateModuleConfig(stack: const []),
        throwsA(isA<AssertionError>()),
      );
    });
  });

  group('GateConfig with nestedState', () {
    test('module config is a GateNestedConfig', () {
      ...
      expect(moduleState, isA<GateModuleConfig>());
      expect(shellState, isA<GateShellConfig>());
    });

    test('nestedState supports pattern matching by kind', () {
      final config = GateConfig<_OtherRoute>(
        mainStack: const [_OtherRoot()],
        nestedState: GateModuleConfig(stack: const [_Cart(), _Shipping()]),
      );
      final result = switch (config.nestedState) {
        GateShellConfig() => 'shell',
        GateModuleConfig(:final stack) => 'module:${stack.length}',
        null => 'none',
      };
    ...
    test('equality includes nestedState', () {
      final a = GateConfig<_OtherRoute>(
        mainStack: const [_OtherRoot()],
        nestedState: GateModuleConfig(stack: const [_Cart()]),
      );
      final b = GateConfig<_OtherRoute>(
        mainStack: const [_OtherRoot()],
        nestedState: GateModuleConfig(stack: const [_Cart()]),
      );
      expect(a, equals(b));
      ...
      final c = GateConfig<_OtherRoute>(
        mainStack: const [_OtherRoot()],
        nestedState: GateModuleConfig(stack: const [_Cart(), _Shipping()]),
      );
  ...
  group('GateModuleMount widget lifecycle', () {
    testWidgets(
    ...
    testWidgets(
      ...
      (tester) async {
        late GateRouter<_CheckoutRoute> capturedRouter;
        ...
        expect(capturedRouter, isA<GateRouter<_CheckoutRoute>>());
...
class _CaptureRouterModule extends RouteModule<_CheckoutRoute> {
  ...
  final void Function(GateRouter<_CheckoutRoute>) onBuild;
