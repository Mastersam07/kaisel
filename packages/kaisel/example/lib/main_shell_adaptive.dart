...
// What this shows
// ---------------
// `KaiselBranch.adaptive` (v0.9) lets a single tab inside a
// `KaiselBranchedShell` use the master-detail absorbing pipeline.
...
//
// 1. **Films** uses `KaiselBranch.adaptive` with a master-detail
...
//
// 2. **Settings** uses a regular `KaiselBranch`. Same layout at all
//    widths.
//
...
/// Films branch routes.
sealed class FilmsRoute extends KaiselRoute {
  const FilmsRoute();
}
      ...
      body: ListView(
        children: const [
          ...
          ListTile(
            ...
            title: Text('About'),
            subtitle: Text('kaisel example app'),
          ),
        ],
      ),
    );
...
KaiselPageResult _filmsAdaptiveBuilder(
  BuildContext context,
  FilmsRoute route,
  KaiselStackContext<FilmsRoute> ctx,
) {
  ...
  return switch ((ctx.previous, route, isWide)) {
    ...
    (_, FilmsList(), _) => const KaiselStandalonePage(_FilmsListScreen()),
    ...
    (FilmsList(), FilmDetail(:final id), true) => KaiselAbsorbingPage(
    ...
    (_, FilmDetail(:final id), _) => KaiselStandalonePage(
...
class _ShellHostState extends State<_ShellHost> {
  late final KaiselRouter<FilmsRoute> _filmsRouter = KaiselRouter<FilmsRoute>(
  ...
  Widget build(BuildContext context) {
    return KaiselBranchedShell(
      ...
      branches: [
        ...
        KaiselBranch<FilmsRoute>.adaptive(
          router: _filmsRouter,
