...
/// host app composes modules by mounting them with [KaiselModuleMount]
...
/// the URL story is parallel (see [KaiselModuleConfig]).
///
/// ```dart
/// sealed class ShopRoute extends KaiselRoute { const ShopRoute(); }
...
/// URL-handled entirely by the host's own [KaiselConfigCodec].
abstract class RouteModule<R extends KaiselRoute> {
  ...
  /// [KaiselStandalonePage] and [KaiselAbsorbingPage]. See
  /// `KaiselRouterDelegate.adaptive` for the canonical example.
  KaiselPageResult buildAdaptivePage(
    BuildContext context,
    R route,
    KaiselStackContext<R> stack,
  ) {
    return KaiselStandalonePage(buildPage(context, route));
  }
  ...
  /// router's guards.
  List<KaiselGuard<R>> get guards => const [];

  /// Optional override of how each route becomes a [Page]. Defaults
  /// to [MaterialPage].
  KaiselPageWrapper<R>? get pageWrapper => null;
...
/// Thin adapter that exposes a [KaiselRouter] as a non-generic
/// [KaiselNestedHandle]. The delegate holds one of these per registered
/// module so it can capture/restore the module's stack without
/// knowing the module's `R`.
class _ModuleHandle<R extends KaiselRoute> implements KaiselNestedHandle {
  _ModuleHandle(this._router);

  final KaiselRouter<R> _router;
  ...
  @override
  KaiselModuleConfig captureConfig() =>
      KaiselModuleConfig(stack: _router.stack);
...
///
/// Creates the module's typed [KaiselRouter] internally; the router
...
/// `KaiselBranch`.
class KaiselModuleMount<R extends KaiselRoute> extends StatefulWidget {
  /// Create a mount around [module].
  const KaiselModuleMount({super.key, required this.module});

  /// The module to mount.
  final RouteModule<R> module;

  @override
  State<KaiselModuleMount<R>> createState() => _KaiselModuleMountState<R>();
}

class _KaiselModuleMountState<R extends KaiselRoute>
    extends State<KaiselModuleMount<R>> {
  late KaiselRouter<R> _router;
  late _ModuleHandle<R> _handle;
  late final GlobalKey<NavigatorState> _navKey = GlobalKey<NavigatorState>(
    debugLabel: 'kaisel-module-$R',
  );
  ...
  void _buildRouter() {
    _router = KaiselRouter<R>.fromStack(
  ...
  @override
  void didUpdateWidget(KaiselModuleMount<R> oldWidget) {
