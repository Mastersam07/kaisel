...
/// widgets.
///
/// Descendants call [KaiselPageScope.of] or [KaiselPageScope.maybeOf] to
...
///
/// The scope's `route` is typed as [KaiselRoute] (the base class).
/// Descendants pattern-match on the actual variant:
///
/// ```dart
/// final scope = KaiselPageScope.of(context);
/// switch (scope.route) {
...
/// child already wrapped in `KaiselPageScope` via
/// `KaiselPageWrapperContext.child`, so a wrapper that does
...
class KaiselPageScope extends InheritedWidget {
  ...
  /// Look up the nearest enclosing [KaiselPageScope] above [context],
  /// or null if there isn't one.
  ///
  /// Returns null if [context] isn't a descendant of a kaisel-managed
  ...
  /// kaisel-managed page. Use [maybeOf] when the widget might be
  ...
  static KaiselPageScope of(BuildContext context) {
    ...
    assert(
      scope != null,
      'KaiselPageScope.of() called from a context without a '
      'KaiselPageScope ancestor. Either use KaiselPageScope.maybeOf(), '
