...
// because KaiselBranchedShell.didChangeDependencies fires during the
...
sealed class _AlphaRoute extends KaiselRoute {
  const _AlphaRoute();
}
...
sealed class _BetaRoute extends KaiselRoute {
  const _BetaRoute();
}
...
class _Parser extends RouteInformationParser<KaiselConfig<_AppRoute>> {
  _Parser(this.router);
...
class _ShellHostWidgetState extends State<_ShellHostWidget> {
  late final KaiselRouter<_AlphaRoute> _alpha = KaiselRouter<_AlphaRoute>(
    initial: const _AlphaHome(),
  );
  late final KaiselRouter<_BetaRoute> _beta = KaiselRouter<_BetaRoute>(
...
void main() {
  ...
  testWidgets('shell-as-initial-route mounts without setState-during-build', (
    ...
    final mainRouter = KaiselRouter<_AppRoute>(initial: const _ShellHost());
