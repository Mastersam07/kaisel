...
class _Codec implements KaiselConfigCodec<_R> {
  ...
  KaiselConfig<_R>? decode(Uri uri) {
    ...
    for (final segment in uri.pathSegments) {
      if (segment == 'home') {
        stack.add(const _Home());
      } else if (segment.startsWith('detail-')) {
        stack.add(_Detail(segment.substring('detail-'.length)));
      } else {
        return null;
      }
    }
...
void main() {
  testWidgets('a codec app restores the main stack across a restart', (
    ...
    await _routerOf(tester).push(const _Detail('x'));
    ...
    expect(find.text('detail x'), findsOneWidget);
    expect(_routerOf(tester).stack, <_R>[const _Home(), const _Detail('x')]);
  });
