...
void main() {
  ...
  group('KaiselSingleStackCodec adapter', () {
    const inner = _SingleCodec();
    const adapted = KaiselSingleStackCodec<_R>(inner);
  ...
  group('KaiselRouteInformationParser', () {
    test('stack constructor decodes deep stacks', () async {
      // v0.5: bare constructor now takes a KaiselConfigCodec. Stack-only
    ...
    test('.single constructor wraps a legacy codec', () async {
      final parser = KaiselRouteInformationParser<_R>.single(
    ...
    test('restoreRouteInformation encodes the configuration', () {
      ...
      final info = parser.restoreRouteInformation(
        KaiselConfig<_R>(mainStack: const [_Home(), _Settings()]),
      );
