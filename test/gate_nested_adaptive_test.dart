...
sealed class _ShopRoute extends GateRoute {
  const _ShopRoute();
}
...
class _AppHomeScreen extends StatelessWidget {
  ...
  final List<GateBranch> branches;
  @override
...
class _AdaptiveShopModule extends RouteModule<_ShopRoute> {
  ...
  GatePageResult buildAdaptivePage(
    ...
    _ShopRoute route,
    GateStackContext<_ShopRoute> stack,
  ) {
...
void main() {
  ...
  group('Shell adaptive: GateBranch.adaptive', () {
    testWidgets(
      ...
      (tester) async {
        ...
        final homeRouter = GateRouter<_HomeRoute>(initial: const _HomeRoot());
        ...
        final mainRouter = GateRouter<_AppRoute>(initial: const _AppHome());
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        final homeRouter = GateRouter<_HomeRoute>(initial: const _HomeList());
        await homeRouter.push(const _HomeDetail('42'));
  ...
  group('RouteModule.buildAdaptivePage default behaviour', () {
    testWidgets(
      ...
      (tester) async {
        ...
        // GateStandalonePage(buildPage(...)). Call it and verify.
        await tester.pumpWidget(
          MaterialApp(
            home: Builder(
              builder: (context) {
                final result = module.buildAdaptivePage(
                  context,
                  const _ShopList(),
                  GateStackContext<_ShopRoute>(
                    stack: const [_ShopList()],
                    position: 0,
                  ),
                );
                expect(result, isA<GateStandalonePage>());
